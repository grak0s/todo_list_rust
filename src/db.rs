...
pub async fn get_todos(client:&Client)->Result<Vec<TodoList>,io::Error>{
    let statement= client.prepare("select * from todo_list oder by id desc").await.unwrap();
...
pub async fn get_items(client:&Client, list_id: i32)->Result<Vec<TodoItem>,io::Error>{
    let statement= client.prepare("select * from todo_items =$1 order by id").await.unwrap();
...
pub async fn check_item(client:&Client, list_id:i32, item_id:i32)-> Result<(), io::Error>{
    ...
    let result = client.execute(&statement, &[&list_id,&item_id])
        .await
        .expect("error cheking todo item");

    match result{
        ref update if *update ==1 =>Ok(()),
        _ =>Err(io::Error::new(io::ErrorKind::Other, "Falied to checki  item"))
    }
}
